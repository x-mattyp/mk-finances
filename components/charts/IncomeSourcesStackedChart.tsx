...
const baseOptions: ChartOptions<"bar"> = {
  ...
  plugins: {
    legend: { position: "top", labels: { color: "#7a7870", boxWidth: 8 } },
    tooltip: {
      callbacks: {
...
type Props = {
  labels: string[];
  series: Record<(typeof INCOME_FIELDS)[number]["key"], number[]>;
  first2026Index: number | null;
  showYearDivider: boolean;
};

export function IncomeSourcesStackedChart({
  labels,
  series,
  first2026Index,
  showYearDivider,
  ...
  return (
    <div className="h-72 w-full">
      <Chart
        ...
        data={{
          labels,
          datasets: INCOME_FIELDS.map((f, i) => ({
            label: f.label,
            data: series[f.key],
